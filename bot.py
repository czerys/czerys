...
ADMIN_ID = 7830830024  # Twój ID admina
GROUP_ID = -1002820595760  # ID Twojej grupy
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.user_data.get('expecting_amount'):
        ...
        if user_id not in user_ltc_addresses:
            await update.message.reply_text("❗ Najpierw dodaj adres LTC w sekcji Portfel.")
            return
    ...
    await update.message.reply_text("❗ Kod Blik musi zawierać 6 cyfr.")
...
async def main_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == 'blik':
        if user_id not in user_ltc_addresses:
            ...
            await context.bot.send_message(chat_id=query.message.chat_id, text="       ❗ Brak Adresu LTC = Brak Wypłaty ❗\n👇 Wklej Swój PRAWIDŁOWY Adres LTC Poniżej👇")
            context.user_data['expecting_ltc_address'] = True
            return
        await context.bot.delete_message(chat_id=query.message.chat_id, message_id=query.message.message_id)
        await context.bot.send_message(chat_id=query.message.chat_id, text="💸 Podaj kwotę\n------------$-----------")
        ...
        await context.bot.delete_message(chat_id=query.message.chat_id, message_id=query.message.message_id)
        await context.bot.send_message(chat_id=query.message.chat_id, text="📈 Twój portfel", reply_markup=InlineKeyboardMarkup(keyboard))

    elif query.data == 'vouche':  # Obsługuje kliknięcie przycisku "Vouche"
        await context.bot.delete_message(chat_id=query.message.chat_id, message_id=query.message.message_id)
        
        # Tutaj dodajemy nowe przyciski: Kanał z voucherami i Powrót
        keyboard = [
            [InlineKeyboardButton("📢  Kanał z vouch'ami", url="https://t.me/FraudBankvouchee")],  # Zmień URL na kanał, do którego chcesz przekierować
            [InlineKeyboardButton("↩️ Powrót", callback_data='back_to_main')]
        ]
        ...
        username = user.username or "Użytkownik bez nazwy"  # <-- dodaj tę linię
        message = (
            f"✦ 👤 @{username} ✦ 💬 Status: {status} ✦\n\n"
            f"📊 Podział: 30/70\n"
            f"🔽 <1000 — 30/70\n"
        ...
        keyboard = [
            [InlineKeyboardButton("🥳 Wrzuć Blika→", callback_data='blik')],
            [InlineKeyboardButton("🏦 Portfel→", callback_data='portfel')],
            [InlineKeyboardButton("🎟 Vouche→", callback_data='vouche')]  # Dodano przycisk Vouche
        ]

        # Usuń starą wiadomość (ze zdjęciem)
        await context.bot.delete_message(chat_id=query.message.chat_id, message_id=query.message.message_id)

        # Wyślij zdjęcie wraz z tekstem i klawiaturą
        with open("logo.jpg", "rb") as photo:
            await context.bot.send_photo(
                chat_id=query.message.chat_id,
                photo=photo,
        ...
        await context.bot.delete_message(chat_id=query.message.chat_id, message_id=query.message.message_id)
        await context.bot.send_message(chat_id=query.message.chat_id, text="✏️ Podaj nowy adres LTC:")
        context.user_data['expecting_ltc_address'] = True
        ...
        keyboard = [[InlineKeyboardButton("↩️ Powrót", callback_data='back_to_main')]]
        await context.bot.delete_message(chat_id=query.message.chat_id, message_id=query.message.message_id)
        await context.bot.send_message(chat_id=query.message.chat_id, text=msg, reply_markup=InlineKeyboardMarkup(keyboard))
...
async def process_transaction(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith(('accept_', 'reject_')):  # Akceptacja lub odrzucenie kodu Blik
        ...
        if action == 'accept':
            # Zatwierdzenie kodu, ale nie aktualizujemy salda jeszcze
            keyboard = [
            ...
            try:
                # Powiadomienie użytkownika o zatwierdzeniu
                await context.bot.send_message(
                    user_id,
        ...
        if step == 'DIP':
            # Zaktualizowanie salda użytkownika po kliknięciu DIP
            user_balances[user_id] = user_balances.get(user_id, 0) + amount  # Zwiększ saldo użytkownika o kwotę transakcji
            calkowity_zysk += amount  # Dodajemy zysk do całkowitych zarobków
